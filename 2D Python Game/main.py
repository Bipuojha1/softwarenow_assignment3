...
pygame.display.set_caption('Nepali 2D Game')
	...
	def check_alive(self):
		if self.health <= 0:
			...
			if self.lives <= 0:
				...
				screen.fill(BG, (10, 70, 100, 30))
				# Update the text displaying the remaining lives
				draw_text(f'Life: {self.lives}', font, WHITE, 10, 70)
...
exit_button = buttons.Button(SCREEN_WIDTH // 2 - 110, SCREEN_HEIGHT // 2 + 50, exit_img, 1)
...
while run:
	...
	if start_game == False:
		...
		draw_text(f'Life: {player.lives}', font, WHITE, 10, 70)
